...
void ParticleEmitter::Initialise(unsigned _maxParticles, unsigned _emitRate, float _lifespanMin, float _lifespanMax, float _velocityMin, float _velocityMax, float _startSize, float _endSize, glm::vec4& _startColor, const glm::vec4& _endColor)
{
    ...
    // There needs to be four vertices per particle to make a quad
    ...
    glEnableVertexAttribArray(0); // This is the position of the particle for the shader
    glEnableVertexAttribArray(1); // This is the colour of the particle for the shader
...
void ParticleEmitter::Emit()
{
    ...
    // Return a deat particle to out available list
    ...
    // Randomise the direction of the particle
...
void ParticleEmitter::Update(float _deltaTime, const glm::mat4& _cameraTransform)
{
    ...
    for(unsigned int i = 0; i < m_firstDead; i++)
    {
        ...
        {
            particle->position += particle->velocity * _deltaTime; // This will need to be changed to use gravity
            ...
            // Set up the billboard transform
